    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
          ...
          <div className="mt-6 flex items-end gap-4">
            ...
            {hasDiscount && (
              <div className="flex flex-col">
                ...
                <span className="text-red-500 text-sm font-bold">חסכת ₪{(product.price - product.salePrice!).toLocaleString()}</span>
              </div>
            )}
          </div>
          ...
          <div className="mt-4 flex items-center gap-2">
            {product.stock > 0 ? (
              <><span className="w-2.5 h-2.5 bg-green-500 rounded-full" /><span className="text-green-700 font-medium text-sm">{product.stock > 5 ? 'במלאי' : `נותרו ${product.stock} בלבד`}</span></>
            ) : (
              <><span className="w-2.5 h-2.5 bg-red-500 rounded-full" /><span className="text-red-600 font-medium text-sm">אזל המלאי</span></>
            )}
          </div>
            ...
            <div className="mt-8 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 border border-gray-200 rounded-xl overflow-hidden">
                  <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-10 h-10 flex items-center justify-center hover:bg-gray-100 font-bold text-gray-600 transition-colors">-</button>
                  <span className="w-12 text-center font-medium">{qty}</span>
                  <button onClick={() => setQty(Math.min(product.stock, qty + 1))} className="w-10 h-10 flex items-center justify-center hover:bg-gray-100 font-bold text-gray-600 transition-colors">+</button>
                </div>
              </div>
              <button onClick={handleAdd} className={`w-full py-4 rounded-2xl font-bold text-lg transition-all ${added ? 'bg-green-500 text-white' : 'bg-orange-500 hover:bg-orange-600 text-white hover:scale-[1.02]'}`}>
          ...
          <div className="mt-8 grid grid-cols-3 gap-3">
            {[{ icon: '🚚', text: 'משלוח חינם מ-₪299' }, { icon: '↩️', text: 'החזרה תוך 30 יום' }, { icon: '🛡️', text: 'אחריות יצרן' }].map(({ icon, text }) => (
          ...
          {product.tags && product.tags.length > 0 && (
            <div className="mt-6 flex flex-wrap gap-2">
              {product.tags.map((tag) => (
                <span key={tag} className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-sm">#{tag}</span>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="mt-16">
        <div className="border-b border-gray-200 flex gap-1">
          {[{ id: 'desc', label: 'תיאור המוצר' }, { id: 'specs', label: 'מפרט טכני' }].map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id as 'desc' | 'specs')}
